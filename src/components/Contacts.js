...
import { View, Text, ListView, TouchableHighlight, Image, StyleSheet, AsyncStorage, ScrollView } from 'react-native';
...
export default class Contacts extends Component {
  ...
  render() {
    ...
    const { contactsSortList } = this.state;
    let contacts = '';

    // contactsSortList !== [] 仍然进入if语句
    if (contactsSortList && contactsSortList.length !== 0) {
      contacts = contactsSortList.map((item, idx) => {
        if (item.length !== 0) {
          return (
            <ContactSection
              key={'key'+idx}
